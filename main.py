...
class OptionWindow(tk.Toplevel):
    def __init__(self, master, *args, **kwargs):
        ...
        self.time_column_entry.set(self.master.thz_handler.column_t, fmt="{0:2d}")
        self.time_column_entry.pack()

        self.thz_column_entry = LabelWithEntry(self, 'Column Signal:', width_label=20, width_entry=5)
        self.thz_column_entry.set(self.master.thz_handler.column_sig, fmt="{0:2d}")
    ...
    def set_values(self):
        self.master.thz_handler.column_t = int(self.time_column_entry.get())
        self.master.thz_handler.column_sig = int(self.thz_column_entry.get())

        if self.master.compare_window is not None and self.master.compare_window.winfo_exists():
            self.master.compare_window.thz_handler.column_t = int(self.time_column_entry.get())
            self.master.compare_window.thz_handler.column_sig = int(self.thz_column_entry.get())
...
class ButtonFrame(tk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        def random_plot():
            ...
            data[:,0] = x
            data[:, 1] = 0.2*np.random.randn(1000) + np.sin(2*np.pi*10*x) + 0.5*np.sin(2*np.pi*20*x) + 0.25*np.sin(2*np.pi*30*x) + np.sin(2*np.pi*40*x)/8 + np.sin(2*np.pi*50*x)/16 + np.sin(2*np.pi*60*x)/32
